...
export default function Sidebar({ 
  ...
  return (
    <aside className={`bg-bg-secondary border-r-3 border-black flex flex-col h-dvh overflow-hidden z-10 max-md:fixed max-md:top-0 max-md:bottom-0 max-md:z-[1000] max-md:shadow-brutal-lg max-md:transition-[left] max-md:duration-250 max-md:ease-[cubic-bezier(0.16,1,0.3,1)] ${isOpen ? 'max-md:left-0' : 'max-md:left-[-345px]'} transition-all duration-300 ${isCollapsed ? 'md:w-0 md:min-w-0 md:border-r-0' : 'w-[340px] min-w-[340px]'}`}>
      <div className={`w-[340px] min-w-[340px] flex flex-col h-full flex-shrink-0 transition-opacity duration-200 ${isCollapsed ? 'md:opacity-0 md:pointer-events-none md:hidden' : 'opacity-100'}`}>
        {/* Sidebar Header */}
